<div class="list-group-item px-4 py-2 d-flex align-items-center border-0 rounded-0" wire:key="{{$bot->id}}">
    <input
        id="checkbox.{{$bot->phone}}"
        class="form-check-input"
        type="checkbox"
        wire:click="{{in_array($bot->id, $selectedBotsIds)? 'rejectBotForGroup': 'selectBotForGroup'}}({{$bot->id}})"
        {{in_array($bot->id, $selectedBotsIds)? 'checked': ''}}
    >
    <label class="form-check-label flex-grow-1 ms-4" for="checkbox.{{$bot->phone}}">
